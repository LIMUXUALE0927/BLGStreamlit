...
champ_dict = {
    champ: lit.get_translation(champ, "zh_CN", object_type="champion")
...
lpl_name = lpl_name[::-1]
lck = leaguepedia_parser.get_tournaments(
    region="Korea", tournament_level="Primary", year=this_year
)
lck_name = [lck[i].overviewPage for i in range(len(lck))]
lck_name = lck_name[::-1]
ldl = leaguepedia_parser.get_tournaments(
    region="China", tournament_level="Secondary", year=this_year
)
ldl_name = [ldl[i].overviewPage for i in range(len(ldl))]
ldl_name = ldl_name[::-1]
worlds = leaguepedia_parser.get_tournaments(
    region="International", tournament_level="Primary", year=this_year
)
...
response = site.api(
    ...
    fields="SG.OverviewPage, SG.Tournament, SG.DateTime_UTC, SG.Patch ,SG.Team1, SG.Team2, SG.WinTeam, SG.Team1Bans, "
           "SG.Team2Bans, SG.Team1Picks, SG.Team2Picks, SG.GameId",
    where=conditions_SG,
)
...
df = (
    df.join(Team1Roles)
    .join(Team2Roles)
    .drop(columns=["Team1PicksByRoleOrder", "Team2PicksByRoleOrder"])
)
...
for i in teams:
    ...
    metrics_i = pd.DataFrame(
        {
            "Team": [i],
            "WinRate": [
                round(
                    len(team_data_i[team_data_i["WinTeam"] == i])
                    / len(team_data_i)
                    * 100,
                    2,
                )
            ]
            if len(team_data_i) > 0
            else "数据不足",
            "WinRate_blue": [
                round(
                    len(team_data_blue[team_data_blue["WinTeam"] == i])
                    / len(team_data_blue)
                    * 100,
                    2,
                )
            ]
            if len(team_data_blue) > 0
            else "数据不足",
            "WinRate_red": [
                round(
                    len(team_data_red[team_data_red["WinTeam"] == i])
                    / len(team_data_red)
                    * 100,
                    2,
                )
            ]
            if len(team_data_red) > 0
            else "数据不足",
            "Wins": [len(team_data_i[team_data_i["WinTeam"] == i])],
            "Total": [len(team_data_i)],
        }
    )
    team_dashboard_data = pd.concat([team_dashboard_data, metrics_i], ignore_index=True)

team_dashboard_data = team_dashboard_data.set_index("Team")
team_dashboard_data["WinRate_rank"] = (
    team_dashboard_data["WinRate"].rank(ascending=False).astype(int)
)

col1, col2, col3, col4, col5 = st.columns(5)
col1.metric(
    "队伍胜率",
    str(team_dashboard_data.loc[team, "WinRate"]) + "%",
    "第{}名".format(team_dashboard_data.loc[team, "WinRate_rank"]),
)
...
team_ban = pd.concat([team_blue_ban, team_red_ban]).reset_index(drop=True)
team_ban = (
    pd.DataFrame(team_ban.groupby(["Champion"])["Count"].sum())
...
team_pick = pd.concat([team_blue_pick, team_red_pick]).reset_index(drop=True)
team_pick = (
    pd.DataFrame(team_pick.groupby(["Champion"])["Count"].sum())
    .sort_values(by="Count", ascending=False)
    .reset_index()
)
...
team_top = pd.concat([team_blue_top, team_red_top]).reset_index(drop=True)
team_top = (
    pd.DataFrame(team_top.groupby(["Champion"])["Count"].sum())
    .sort_values(by="Count", ascending=False)
    .reset_index()
)
...
team_jug = pd.concat([team_blue_jug, team_red_jug]).reset_index(drop=True)
team_jug = (
    pd.DataFrame(team_jug.groupby(["Champion"])["Count"].sum())
    .sort_values(by="Count", ascending=False)
    .reset_index()
)
...
team_mid = pd.concat([team_blue_mid, team_red_mid]).reset_index(drop=True)
team_mid = (
    pd.DataFrame(team_mid.groupby(["Champion"])["Count"].sum())
    .sort_values(by="Count", ascending=False)
    .reset_index()
)
...
team_bot = pd.concat([team_blue_bot, team_red_bot]).reset_index(drop=True)
team_bot = (
    pd.DataFrame(team_bot.groupby(["Champion"])["Count"].sum())
    .sort_values(by="Count", ascending=False)
    .reset_index()
)
...
team_sup = pd.concat([team_blue_sup, team_red_sup]).reset_index(drop=True)
team_sup = (
    pd.DataFrame(team_sup.groupby(["Champion"])["Count"].sum())
    .sort_values(by="Count", ascending=False)
    .reset_index()
)
