...
st.markdown("本次查询时间段: " + (datetime.today() - timedelta(days=7)).strftime(
...
for i in namelist:
    try:
        ...
        matchlist = lol_watcher.match.matchlist_by_puuid(region='asia', puuid=puuid, type='ranked', count = 100,
                                                         start_time=start, end_time=today)
